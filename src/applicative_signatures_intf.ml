...
module type Applicative_general = sig
  ...
  val all_unit
    :  (unit -> (unit, 'd, 'e) t -> _, unit -> 'at -> _, [> many_getter ]) accessor
    -> 'at
    -> (unit, 'd, 'e) t
...
module type Applicative_s3 = Applicative_general with type 'a optional_args := 'a

module type Applicative_without_return_s3 =
  Applicative_without_return_general with type 'a optional_args := 'a

module type Applicative_s2 = sig
  type ('a, 'e) t

  include Applicative_s3 with type ('a, _, 'e) t := ('a, 'e) t
end
...
module type Monad_s2 = sig
  ...
  include Monad_s3 with type ('a, _, 'e) t := ('a, 'e) t
end
